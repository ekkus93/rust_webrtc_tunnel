//! Answer-side multiplex loop: drives the data-channel/stream-event/target-connect/
//! writer select loop, resolves inbound OPEN frames against the forward table,
//! connects to the local target off the dispatcher, and dispatches inbound frames
//! (OPEN, DATA, CLOSE, ERROR, PING/PONG) for the answer role.
...
pub(crate) async fn handle_answer_frame(
    frame: TunnelFrame,
    forward_table: &ForwardTable,
    remote_peer_id: &PeerId,
